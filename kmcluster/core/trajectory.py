...
class trajectory:
    ...
    def step(self, index, draw_crit, rand_state, neg_log_time_sample, time_stop=10e9):
        ...
        new_state, time_to_transition = self.draw_new_state(
            index, draw_crit, rand_state, neg_log_time_sample
        )
    ...
    def states_at_times(self, times):
        ...
        """
        states = [
            self.states[brisk.bisect_right(self.transition_times, time) - 1]
            for time in times
        ]
        return states
...
class trajectory_minimum:
    ...
    def batched_step(
        ...
        if (
            self.current_time > time_check
        ):  # this means that at LEAST THE NEXT SAMPLING POINT HIT
            if probe:
            ...
            while self.current_time > time_check and time_stop >= time_check:
                ...
                if sample_trigger == 0:
                    ...
                    probe_states.append(
                        int(new_states[sample_trigger - 1])
                    )  # for surre this the error
