...
class boltz(initializer): 
    def __init__(self, size, T, energies): 
        ...
        self.probabilities_normalized = [p/self.sum_probabilities for p in self.probabilities]
        self.population = None
    ...
    def get_init_populations(self):
        ...
        if self.population is None:
            ...
            for i in range(self.size):
                ind = random.choice(range(len(self.probabilities)), p=self.probabilities_normalized)
                population[ind] = population[ind] + 1
