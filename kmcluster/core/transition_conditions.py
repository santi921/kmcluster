...
class rfkmc:
    ...
    def call_batched(self, state, rand_states, neg_log_time_samples):
        ...
        """
        return_states, time_to_transitions = self.inst_ret_vars(
            len(neg_log_time_samples)
        )
        ...
        for i in range(len(rand_states)):
            ...
            sum_rates, rates_cum = (
                self.sum_rates[last_state],
                self.cum_rates[last_state],
            )
            ...
            if sum_rates == 0:
                ...
                if i == 0:
                    ...
                    time_to_transitions[i] = div_w_index(
                        neg_log_time_samples, sum_rates, i
                    )
                else:
                    time_to_transitions[i] = div_and_sum(
                        neg_log_time_samples[i], sum_rates, time_to_transitions[i - 1]
                    )  # expensive
